...
export interface StatusSelectProps {
  ...
  onChange: (next: StudentStatus | null) => void;
}

const NONE_VALUE = "__any__";

export function StatusSelect({ value, onChange }: StatusSelectProps): React.ReactElement {
  return (
    <div className="relative">
      <select
        ...
        onChange={(e) => {
          ...
          onChange(next === NONE_VALUE ? null : (next as StudentStatus));
        }}
        className={cn(
      ...
      >
        <option value={NONE_VALUE}>Status: Any</option>
        {studentStatusValues.map((key) => (
          <option key={key} value={key}>
            Status: {studentStatusLabels[key]}
          </option>
        ))}
      </select>
      <ChevronDown
