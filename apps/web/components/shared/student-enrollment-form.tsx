...
export interface StudentEnrollmentFormProps {
  /**
   * Optional override for the list of classes the form lets the user pick
   * from. When omitted we fetch the full school-wide list via
   * API_ENDPOINTS.classes (the admin default). Callers that need to scope
   * the picker — notably the class-teacher flow, which can only enroll into
   * its own class(es) — should pass this explicitly.
   */
  classes?: ClassItem[];
  /** Header eyebrow label (e.g. "Admin operations", "Teacher tools"). */
  eyebrow: string;
  ...
  description: string;
  /**
   * Route used by the "Back to Students" button and Cancel action. Keeps
   * the admin flow returning to /admin/students and the teacher flow
   * returning to /teacher/students.
   */
  listHref: string;
  /** Builds the profile URL for the newly enrolled student. */
  profileHref: (studentId: string) => string;
  /**
   * Shown when no classes are available — typically only relevant for the
   * admin flow. When omitted we hide the "Manage Classes" fallback button.
   */
  manageClassesHref?: string;
  /**
   * Copy shown in the empty-classes warning banner. Teacher flow wants a
   * different message ("You are not a class teacher yet"), so it's
   * parameterised.
   */
...
export function StudentEnrollmentForm({
  ...
  manageClassesHref,
  emptyClassesMessage,
}: StudentEnrollmentFormProps): React.ReactElement {
  const router = useRouter();
  const [fetchedClasses, setFetchedClasses] = React.useState<ClassItem[]>([]);
  ...
  // When the caller scopes the class list (teacher flow), auto-select the
  // only eligible class so the picker is pre-filled. The Select still
  // renders so the user can see which class they're enrolling into.
  React.useEffect(() => {
    if (!classesProp) return;
    if (classId) return;
    if (classesProp.length !== 1) return;
    setClassId(classesProp[0]!.id);
  }, [classesProp, classId]);
            ...
            <div className="max-w-2xl space-y-4">
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                <Button
                  type="button"
                  onClick={() =>
                    router.push(profileHref(enrolled.studentId))
                  }
                >
